...
class Amrap_Helper_Workout_Text extends StatelessWidget {
  Amrap_Helper_Workout_Text({super.key, required this.amrapModel});
  AmrapModel? amrapModel;
  @override
  Widget build(BuildContext context) {
    // amrapModel ?? AmrapWorkouts().randomAmrapWorkout();
    // print('Random Amrap Model: ${amrapModel!.description!.split(',').join('')} ');
    return Container(
        ...
        children: [
          ...
          TextButton(
            onPressed: () async {
              ...
              AmrapModel newAmrapWorkout = AmrapModel(duration: duration * 60, rounds: 0);
              BlocProvider.of<AMRAPBloc>(context).add(CreateAmrapModel(duration: duration * 60, rounds: 0));
              /// Original
              //BlocProvider.of<AMRAPBloc>(context).add(AmrapUpdate(duration: duration, status: AMRAP_Status.paused));
              /// Test
              BlocProvider.of<AMRAPBloc>(context).add(AmrapUpdate(duration: duration, status: AMRAP_Status.paused, description: '${amrapModel?.description}\n\nNotes:' ));
